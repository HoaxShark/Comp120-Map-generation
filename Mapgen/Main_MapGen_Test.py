...
class MapClass:
    ...
    Terrain = [[0 for x in range (MAPCONST.SIZE_X_MAX)]for y in range (MAPCONST.SIZE_X_MAX)] # Create array for terrain
        ...
        for y in range(0,MAPCONST.SIZE_Y_MAX):
            for x in range(0, MAPCONST.SIZE_X_MAX):
                ...
                if number == 42: # If tile number is 42
                    self.Terrain[x][y] = 1
                    #temp_riv = pygame.image.load(MAPCONST.RIVER_TILE[0]).convert() # Load river start image
                    #surf.blit(temp_riv,(x*MAPCONST.BLOCK_SIZE,y*MAPCONST.BLOCK_SIZE))
        MapClass.river_flow(surf)
        return surf

    def river_flow(self,surf):
        for y in range(0,MAPCONST.SIZE_Y_MAX):
            for x in range(0, MAPCONST.SIZE_X_MAX):
                if self.Terrain[x][y] is not 0:
                    break
                elif self.Terrain[x][y-1] is 1: # If above tile is river start
                    ran_tile = random.randint(3, 5)
                    self.Terrain[x][y] = ran_tile




                river_length = random.randint(MAPCONST.RIVER_MIN,MAPCONST.RIVER_LENGTH) # Decide random river length between min and max
                for i in range(0, river_length - 1):
